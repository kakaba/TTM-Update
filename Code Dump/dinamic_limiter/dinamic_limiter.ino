...
unsigned long blinkTimeHigh = 1500;
...
void reboot() {
  ...
  delay(200);
  Serial.println(F("this never happens"));
}
...
void slowDown() {
  ...
  if ( overSpeedMode == 1 && millis() > slowDownTime + overSpeedTime ) {
    ...
    if (millis() > pwmInterval + pwmTimeSet && pwmPower > configPwm) {
    ...
    if (millis() > (pwmDelay + pwmDelaySet) && pwmPower <= configPwm) {

      pwmDelaySet = millis();

      if ( pwmPower == configPwm ) {
        pwmPower = 0;
        pwmDelay = configDelayLow;
        Serial.println(" pwm 0 ");
      }
      else {
        pwmPower = configPwm;
        pwmDelay = configDelayHigh;
        Serial.println(" pwm : ");
